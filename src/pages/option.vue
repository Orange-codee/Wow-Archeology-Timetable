...
export default {
  data() {
    return {
      ...
      // timeInterval: 1209600, //間隔14天輪換下一個，即1209600秒
      firstStr: "2025/1/9",
      firstDate: "",
      today: 0,
      testStr: "",
  ...
  methods: {
    getDate() {
      ...
      this.today =
        this.testStr !== ""
          ? new Date(this.testStr).getTime() / 1000
          : Math.floor(new Date().getTime() / 1000);
      console.log("today", this.today);
      //將初始日期轉換為時間戳記
      this.firstDate = new Date(this.firstStr).getTime() / 1000;
      console.log("firstDate", this.firstDate);
    },
    countDate() {
      const timeSpan = Math.floor((this.today - this.firstDate) / 86400);
      const countEvent = Math.floor(timeSpan / 14) + 1;
      this.eventDay = (timeSpan % 14) + 1;
      console.log(
        "間隔天數",
        timeSpan,
        "第幾個事件",
      ...
      if (!countEvent % 13) {
        ...
        this.index = (countEvent % 13) - 1;
      }
      console.log("index", this.index);
    },
    getNewEventArray() {
      // console.log(this.event.slice(this.index));
      // console.log(this.event.slice(0,this.index));
      this.newEventArray = this.event.slice(this.index);
      this.event.slice(0, this.index).forEach((item) => {
        this.newEventArray.push(item);
      });
    },
    countTime() {
      this.newEventArray[0].startingDate =
        this.today - this.eventDay * 86400 + 1 * 86400;
      this.newEventArray[0].dueDate =
        this.newEventArray[0].startingDate + 14 * 86400 - 1 * 86400;
      for (let i = 1; i < 13; i++) {
        this.newEventArray[i].startingDate =
          this.newEventArray[i - 1].startingDate + 14 * 86400;
        this.newEventArray[i].dueDate =
          this.newEventArray[i - 1].dueDate + 14 * 86400;
      }
